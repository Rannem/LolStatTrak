...
export class ClubService {
  ...
  /** Everything the club page needs in one round-trip. */
  getOverview(clubId: string): Observable<ClubOverviewPage> {
    return this.http
      .get<ClubOverviewPage>(`${this.base}/${clubId}/overview`, { withCredentials: true })
      .pipe(tap((data) => this.overviewCache.set(clubId, { at: Date.now(), data })));
  }

  /** Returns a recent cached overview (if any) for instant first paint; caller should still revalidate. */
  peekOverview(clubId: string): ClubOverviewPage | null {
    const hit = this.overviewCache.get(clubId);
    return hit && Date.now() - hit.at < ClubService.OVERVIEW_TTL_MS ? hit.data : null;
  }

  /** Fire-and-forget warm-up, e.g. on hovering a club card. */
  prefetchOverview(clubId: string): void {
    if (this.peekOverview(clubId)) return;
